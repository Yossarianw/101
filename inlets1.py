# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 11:54:47 2022
...
tqdm = get_tqdm()
im = ps.generators.blobs(shape=[300, 300], porosity=0.2, blobiness=2)
...
def ibip(im, inlets=inlets, dt=None, maxiter=100):
    ...
    ip = ps.filters.ibip(im=im, inlets=inlets, maxiter=1)

    fig, ax = plt.subplots(1, 1, figsize=[6, 6])
    ax.imshow(ip.inv_sequence/im, origin='lower')
    ax.axis(False);
    
    ip = ps.filters.ibip(im=im, inlets=inlets, maxiter=10)

    fig, ax = plt.subplots(1, 1, figsize=[6, 6])
    ax.imshow(ip.inv_sequence/im, origin='lower')
    ax.axis(False);
    
    ip = ps.filters.ibip(im=im, inlets=inlets, maxiter=15)

    fig, ax = plt.subplots(1, 1, figsize=[6, 6])
    ax.imshow(ip.inv_sequence/im, origin='lower')
    ax.axis(False);
    
    ip = ps.filters.ibip(im=im, inlets=inlets, maxiter=20)

    fig, ax = plt.subplots(1, 1, figsize=[6, 6])
    ax.imshow(ip.inv_sequence/im, origin='lower')
    ax.axis(False);
    
    ip = ps.filters.ibip(im=im, inlets=inlets, maxiter=30)
    ...
    ax.imshow(ip.inv_sequence/im, origin='lower')
    ax.axis(False);
ibip(im = ps.generators.blobs(shape=[300, 300], porosity=0.2, blobiness=2))